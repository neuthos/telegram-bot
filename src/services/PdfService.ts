...
export class PDFService {
  ...
  private generateHTMLTemplate(
    ...
    const fullAddress = application.postal_code
      ...
      : application.address;
    const addressFormatted = this.formatAddress(fullAddress || "");
    ...
    const signatureDisplay = signaturePhoto
      ? `<img src="${signaturePhoto.file_url}" alt="Signature" class="max-w-full max-h-full object-contain mx-auto" style="height: 60px" />`
            ...
            <div class="mt-2 text-xs">
              ...
              <div class="flex flex-col items-start">
                ...
                <div class="mt-1 px-5 flex items-center justify-between w-full">
                  <div>
                    <div class="mb-3 flex mx-auto" style="height: 60px">
                      ${signatureDisplay}
                    </div>
  ...
  private generatePhotoPages(photos: KYCPhoto[]): string {
    ...
    const filteredPhotos = photos.filter(
      (photo) => photo.photo_type !== "signature"
    );
    ...
    const orderedTypes = ["location_photos", "id_card", "bank_book"];

    orderedTypes.forEach((photoType) => {
      const photoList = groupedPhotos[photoType];
      if (!photoList) return;
      ...
      photoList.forEach((photo, index) => {
        ...
        <div class="page bg-white p-6 border border-gray-400 flex flex-col">
          ...
          <div class="text-center text-xs text-gray-500 mt-4">
            Halaman ${++photoCount} dari ${filteredPhotos.length} foto
          </div>
        </div>
      `;
      });
